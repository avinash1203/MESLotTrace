...
Public Class MastProdCapacity
    ...
    Protected Sub SaveData()
        ...
        sqlstr = "SELECT COUNT(*) FROM MAST_PROD_CAPACITY WHERE item_cd =@item_cd AND line_id=@line_id AND proc_flow_id = @proc_flow_id AND CNCL_FLG = 0"
        cmd = New SqlCommand(sqlstr, myconnection)
        cmd.Parameters.AddWithValue("@proc_flow_id", ddlPFID.SelectedValue)
        cmd.Parameters.AddWithValue("@item_cd", txtIC.Text)
    ...
    Private Sub InsertProdCap()
        ...
        sqlstr = "INSERT INTO [dbo].[MAST_PROD_CAPACITY] 
                   ...
                   ,[notes])" +
                          " VALUES(" +
                            "@cmp_cd,@plnt_cd,@item_cd,@proc_flow_id,@line_id,@shift_num,@production_capacity,@proc_seq," +
        ...
        cmd.Parameters.AddWithValue("@item_cd", txtIC.Text.Trim)
        cmd.Parameters.AddWithValue("@proc_flow_id", ddlPFID.SelectedValue)
        cmd.Parameters.AddWithValue("@line_id", ddlLineId.SelectedValue)
        cmd.Parameters.AddWithValue("@shift_num", txtNS.Text)
    ...
    Protected Sub UpdateProdCap()
        ...
        sqlstr = "UPDATE MAST_PROD_CAPACITY" +
                       ...
                       ,[upd_id]=@upd_id
                       ,[upd_utc]=@upd_utc
                       ,[notes]=@notes Where item_cd =@item_cd AND line_id=@line_id AND proc_flow_id = @proc_flow_id"
    ...
    Protected Sub gvContent_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvContent.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            Dim row As GridViewRow = gvContent.Rows(rowIndex)
            Dim itemcd As String = row.Cells(1).Text
            Dim lineid As String = row.Cells(2).Text
            Dim pflowid As String = row.Cells(3).Text
            GetSelected(itemcd, lineid, pflowid)

        End If
    End Sub

    Protected Sub GetSelected(ByVal itemcd As String, lineid As String, pflowid As String)
        connstr = System.Configuration.ConfigurationManager.ConnectionStrings("MyDatabase").ConnectionString
        Dim sqlstr As String
        Dim myConnection As SqlConnection

        sqlstr = "SELECT cmp_cd,plnt_cd,item_cd,line_id,proc_flow_id,shift_num,production_capacity,priority_order,line_sym,proc_sym,notes 
                                      from MAST_PROD_CAPACITY 
                                      WHERE item_cd=@itemcd and line_id=@lineid and proc_flow_id = @pflowid AND CNCL_FLG = 0"
        ...
        cmd.Parameters.AddWithValue("@pflowid", pflowid)
    ...
    Protected Sub fillDetails(value As DataTable)
        ...
        'End If
        ' Class1.SetDropDownVale(DataEntryScr, "ddlIC", value.Rows(0).Item(2))


        DirectCast(DataEntryScr.FindControl("txtIC"), TextBox).Text = value.Rows(0).Item(2)
        ...
        Class1.SetDropDownVale(DataEntryScr, "ddlLS", value.Rows(0).Item(8))

        DirectCast(DataEntryScr.FindControl("txtPS"), TextBox).Text = value.Rows(0).Item(9)
        If IsDBNull(value.Rows(0).Item(10)) Then
            DirectCast(DataEntryScr.FindControl("txtRe"), TextBox).Text = String.Empty
        Else
    ...
    Protected Sub SoftDeleteReason()
        ...
        sqlstr = "UPDATE MAST_PROD_CAPACITY" +
                        " SET CNCL_FLG = 1  WHERE item_cd =@item_cd AND line_id=@line_id AND proc_flow_id = @proc_flow_id"
        myconnection.Open()
        cmd = New SqlCommand(sqlstr, myconnection)
        cmd.Parameters.AddWithValue("@proc_flow_id", ddlPFID.SelectedValue)
        cmd.Parameters.AddWithValue("@item_cd", txtIC.Text)
